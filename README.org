...
The primary interface to one's music library is through the filesystem, by selecting a file to play, or by adding files and directories to a queue.  Although MPD is not required, support is provided for finding files from a local MPD server's library using MPD's metadata searching.  The package also supports playback from OpenSubsonic-compatible servers such as Navidrome or Gonic.
...
For use with an OpenSubsonic-compatible music server, set
~listen-infrasonic-url~.  Credentials are read from ~auth-source~ using that
host.  If multiple accounts share the host, also set
...
- From a Subsonic server: ~listen-queue-add-from-subsonic~.  Prompts for search, starred or playlist to add to the selected queue.
*** Queue buffer
...
- From a Subsonic server: ~listen-library-from-subsonic~.  Prompts for search, starred or playlist.
**** Commands
...
- [[https://mpv.io/][MPV]] support (works asynchronously, to improve performance and avoid blocking Emacs).
- Command ~listen-status~, which shows a status buffer (contributions to it are welcome).
...
*Changes*
- Improve performance of adding large numbers of tracks to large queues (using a hash table for deduplication).
