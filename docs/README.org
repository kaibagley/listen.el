...
For use with an OpenSubsonic-compatible music server, set ~listen-infrasonic-url~.  Credentials are read from ~auth-source~ using that host.  If multiple accounts share the host, also set ~listen-infrasonic-username~.
